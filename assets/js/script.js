// Global variables
const apiKey = '7e69c318046ad563050daf4de920f434';
var city = '';
var search = '';
var lat;
var lon;
...
function renderSearchHistory() {
    ...
    for (var i = 0; i <cities.length; i++) {
      ...
      document.getElementById(cities[i]).addEventListener('click', handleSearchHistoryClick);
    }
  }
  ...
  function renderCurrentWeather(city, weather) {
    ...
    // Text content of appended element
  ...
  // Function to display 5 days worth of forecast
  function renderForecast(dailyForecast) {
    ...
    // append elements for document sand styles
    ...
    for (var i = 0; i < 5; i++) {
      ...
      // appends the elements to documents
      ...
      //  elements current data and content 
    ...
    fetch(geoUrl)
      .then(function (response) {
        if (response.ok) {
          response.json()
            .then(function (data) {
              // Returns nothing if there is no information for that city
              if (data.length === 0) {
  ...
  // collects value of the User Input when search is clicked
  ...
  // chooses value of button clicked 
